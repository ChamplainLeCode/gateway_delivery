...
public class TransactionResource {
	...
	public Object newCommande(@RequestBody Transactions transac) {  
		...
		List<AutomateAgents> agents = new LinkedList<>();
                ...
                agentListLock.lock();
                try{
                    WorkSpaceService.map.forEach((sessionId, pipeEntry) -> {
...
//                    });
                    ...
                    if(agents.size() > 0){
                        ...
                        if(availableAgents.size() > 0) {
                        	...
                        	if(agent == null)
                        		activity.setLog(activity.getLog()+WorkSpaceService.LOG_SEPARATOR+"Tous les agents actifs pouvant résoudre cette transaction sont désactivés");
                        	
//                        	agent = availableAgents.get(0);
                        }else {
                        	activity.setLog(activity.getLog()+WorkSpaceService.LOG_SEPARATOR+"Aucun agent actif trouvé pour opérateur téléphonique");
                        }
                    }else
                    	activity.setLog(activity.getLog()+WorkSpaceService.LOG_SEPARATOR+"Aucun agent actif trouvé");
